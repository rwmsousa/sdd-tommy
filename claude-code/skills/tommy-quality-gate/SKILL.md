...
- Use Tommy MCP `quality-check(repoRoot, targetPath, fix, cache)` if available.
...
1. Use Tommy MCP `complexity-check(repoRoot, patterns, maxComplexity, maxLines)` if available.
...
2. If present, run `sonar-run(repoRoot, targetPath)` via Tommy MCP.
...
1. Read the checklist created by codegen agent (`.tommy/specs/[SPEC_FOLDER]/checklists/[CHECKLIST_FILE].md`).
