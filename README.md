...
│                        mcp/ (catalog + wiring), sonar-project-reference.properties,
...
│   ├── agents/, commands/, skills/, hooks/, settings.json (mcp.json: registered via claude mcp add)
│
...
- **Tommy Specify**: takes a general task (feature) and creates the specification (`spec.md`), eliciting requirements from the user and closing with an independent PM-lens review.
...
- **Tommy Codegen**: takes a detailed execution plan and generates the code following it, drawing on the project's best practices and code patterns, and closes with a spec→code acceptance traceability matrix.
...
In Claude Code, Specify and Prompt are commands that orchestrate in the main conversation (requirements elicitation is a truly interactive skill there), delegating to role-restricted subagents where fresh context matters: `tommy-architect` (architecture design) and `tommy-product-review` (independent spec review + acceptance traceability) — see `claude-code/README.md`. In Copilot and Cursor, the same responsibilities are condensed into 3 files per tool, with the PM review as a self-enforced role switch (see `github-copilot/README.md` and `cursor/README.md`).
...
1. Run `npx sdd-tommy@latest` in your project's folder (or anywhere, to install just Claude Code globally) — the interactive installer asks which tool(s) to install (Claude Code, Cursor, GitHub Copilot — you can pick more than one) and puts the right files in the right place on its own: Claude Code goes to `~/.claude/` (global, once, applies to every project), Cursor and Copilot go to `.cursor/`/`.github/` of the current project. It also asks, once per project, how to wire the project's MCP servers (`.tommy/mcp.json` is the canonical source; generating `.mcp.json` at the root is the recommended option for native Claude Code auto-load) and registers the global `context7` server via `claude mcp add --scope user`. All JSON configs are merged with what you already have — never blindly overwritten, always with an automatic backup before any change. Running it again later is safe (it updates without duplicating or losing your customization).
...
4. Trigger the **Specify** phase — describe the feature clearly and in detail (what needs to be done, what the goal is, what the constraints are):
   - **Claude Code**: `/tommy-specify <feature description>`
...
5. Trigger the **Prompt** phase, referencing the specification created in step 4:
   - **Claude Code**: `/tommy-prompt <path to spec.md>`
...
1. Create a clear, detailed general task for the Specify phase.
    ...
    - To start it: `/tommy-specify <task description>` in Claude Code and Copilot Chat, or `@tommy-specify` in Cursor — see "How to Use It?" above for the full per-tool syntax.
...
3. Reference the specification in the Prompt phase (`/tommy-prompt` in Claude Code and Copilot, `@tommy-prompt` in Cursor), to create the detailed execution plan (including architecture).
...
│   ├── agents/, commands/, skills/, hooks/, settings.json (mcp.json: registrado via claude mcp add)
│
...
- **Tommy Specify**: recebe uma tarefa geral (feature) e cria a especificação (`spec.md`), elicitando requisitos do usuário e fechando com uma revisão independente com lente de PM.
- **Tommy Prompt**: recebe uma especificação aprovada e cria o plano de execução detalhado, incluindo a arquitetura da feature.
- **Tommy Codegen**: recebe um plano de execução detalhado e gera o código seguindo o plano, buscando por melhores práticas e padrões de código do projeto, e fecha com a matriz de rastreabilidade spec→código.
...
No Claude Code, Specify e Prompt são comandos que orquestram na conversa principal (a elicitação de requisitos é uma skill realmente interativa lá), delegando a subagentes com tools restritos por papel onde contexto novo importa: `tommy-architect` (desenho de arquitetura) e `tommy-product-review` (revisão independente da spec + rastreabilidade de aceite) — ver `claude-code/README.md`. No Copilot e no Cursor, as mesmas responsabilidades ficam condensadas em 3 arquivos por ferramenta, com a revisão de PM como troca de papel autodisciplinada (ver `github-copilot/README.md` e `cursor/README.md`).
...
1. Rode `npx sdd-tommy@latest` na pasta do seu projeto (ou em qualquer lugar, para instalar só o Claude Code globalmente) — o instalador interativo pergunta para qual(is) ferramenta(s) instalar (Claude Code, Cursor, GitHub Copilot — pode escolher mais de uma) e coloca os arquivos certos no lugar certo sozinho: Claude Code vai para `~/.claude/` (global, uma vez só, vale pra todos os projetos), Cursor e Copilot vão para `.cursor/`/`.github/` do projeto atual. Ele também pergunta, uma vez por projeto, como ligar os MCPs do projeto (`.tommy/mcp.json` é a fonte canônica; gerar `.mcp.json` na raiz é a opção recomendada para carregamento nativo no Claude Code) e registra o `context7` global via `claude mcp add --scope user`. Todas as configs JSON são mescladas com o que você já tem — nunca sobrescritas às cegas, sempre com backup automático antes de qualquer mudança. Rodar de novo mais tarde é seguro (atualiza sem duplicar nem perder customização sua).
...
4. Acione a fase **Specify** — descreva a feature de forma clara e detalhada (o que deve ser feito, qual é o objetivo, quais são as restrições):
   - **Claude Code**: `/tommy-specify <descrição da feature>`
...
5. Acione a fase **Prompt**, referenciando a especificação criada no passo 4:
   - **Claude Code**: `/tommy-prompt <caminho do spec.md>`
...
1. Crie uma tarefa geral clara e detalhada para a fase Specify.
    ...
    - Para iniciar: `/tommy-specify <descrição da tarefa>` no Claude Code e no Copilot Chat, ou `@tommy-specify` no Cursor — ver "Como utilizar?" acima para a sintaxe completa por ferramenta.
...
3. Referencie a especificação na fase Prompt (`/tommy-prompt` no Claude Code e no Copilot, `@tommy-prompt` no Cursor), para criar o plano de execução detalhado (incluindo arquitetura).
