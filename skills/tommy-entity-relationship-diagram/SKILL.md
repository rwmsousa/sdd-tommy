...
> **Important:** This skill does NOT generate diagrams directly. It provides the modeling rules and standards. Diagram creation and rendering must be done using the `tommy-plantuml-diagram` skill with PlantUML ER syntax.
...
Before generating or fixing an ERD, the `tommy-plantuml-diagram` skill must be used for diagram creation. Refer to the PlantUML ER reference (`references/er_diagrams.md` inside the plantuml skill) for syntax details.
...
- Diagrams must be generated in **PlantUML ER syntax** using the `tommy-plantuml-diagram` skill.
...
2. Final PlantUML ER diagram (generated via `tommy-plantuml-diagram` skill).
