...
- `makuco-architect`: Use this agent to design the implementation architecture of the feature based on the requirements. It will help define boundaries, components, contracts, and data model decisions.
...
2. Knowledge chain: Research the project documentation and resources. Use `makuco-project-research` skill if needed.
3. Call `makuco-architect` agent sending all relevant information and asking for an architecture plan.
4. Create the raw prompt file using `.makuco/scripts/create-new-prompt.sh --json --spec-folder ".makuco/specs/[spec-folder]"`
    - Always pass spec-folder with a folder in .makuco/specs, never the root.
5. Fill the prompt using `.makuco/templates/prompt-template.md`.
6. Validate checklist, read `.makuco/specs/[SPEC_FOLDER]/checklists/[CHECKLIST_FILE].md` and ensure the plan meets all criteria.
