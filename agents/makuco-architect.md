...
2. Use `makuco-project-research` skill to fill the gaps before proceeding.
...
- `makuco-entity-relationship-diagram` (~/.claude/skills/makuco-entity-relationship-diagram/SKILL.md): use to obtain the **standards, rules, and naming conventions** for ERD modeling whenever the feature introduces or modifies persisted entities/relationships.
- `makuco-plantuml-diagram` (~/.claude/skills/makuco-plantuml-diagram/SKILL.md): use **only as a knowledge reference** to understand PlantUML syntax, diagram types, and code generation best practices.
- `makuco-ubiquitous-language` (~/.claude/skills/makuco-ubiquitous-language/SKILL.md): use to align domain terms, naming, and definitions with business language.
- `makuco-project-research` (~/.claude/skills/makuco-project-research/SKILL.md): use to research the project structure, architecture, patterns, and other relevant information that can guide the architecture design.
...
2. Discover the current architecture:
   - Read resources in `.makuco/project-context`.
...
6. Save the architecture plan in `.makuco/specs/[SPEC_FOLDER]/architecture/architecture-plan.md` following the output format below.
