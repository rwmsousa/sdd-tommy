...
Create or refresh `AGENTS.md` **at the project root** — a deliberate exception to "everything lives inside `.tommy/`", because that's the only location Copilot (and Cursor) discover it natively. Generate it from `.tommy/templates/agents-md-template.md`. Keep it short (10-20 lines) and free of anything project-sensitive — it's navigation to `.tommy/TOMMY.md`, not a copy of it. (The only other file Tommy may place at the root is the optional `.mcp.json` from Step 5.1, when MCP wiring is `root-file`.)

## Step 5 — Capabilities (opt-in, always confirm with the user first)

1. **MCP**: follow `.tommy/templates/mcp/mcp-catalog.md`. Propose the catalog servers that match the stack (context7 always; playwright when the stack has a frontend UI), each with its "why"; add confirmed ones to `.tommy/mcp.json`. Project them **only into the tool-native files for tools actually in use in this project** — always generate `.vscode/mcp.json` (note its `servers` + `type: stdio` format, per the catalog; Copilot is in use here, since this prompt lives in `.github/prompts/`); generate `.mcp.json` at the project root only if Claude Code is also in use (evidence: `.tommy/config.json` already has `mcp.wiring` set, or a root `.mcp.json`/`~/.claude/` setup is otherwise known) — if so and `mcp.wiring` is unset, ask the user (`root-file` vs `tommy-only`) and persist it; generate `.cursor/mcp.json` only if `.cursor/rules/tommy-core.mdc` already exists (evidence Cursor is set up). Never create a `.cursor/` folder or a root `.mcp.json` as a side effect of this step — a Copilot-only project must stay that way. All writes are non-destructive merges.
